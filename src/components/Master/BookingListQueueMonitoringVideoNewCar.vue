<template>
  <div :style="{ backgroundColor: '#FBD743', height: '100%', width: '100%' }" class="respons_bg">
    ...
    <div class="row d-flex justify-content-center px-4 pb-3">
      <div v-bind:class="{'bg-primary': groupIndex % 2 === 0, 'bg-secondary': groupIndex % 2 !== 0}" class="col-4 p-0" v-for="(group, groupIndex) in groupedData" :key="groupIndex">
      <!-- <div class="col-4" v-for="(group, groupIndex) in groupedData" :key="groupIndex" style="background-color: red;"> -->
        <div class="d-flex flex-column justify-content-center px-2">
          <BookingUseCar
            :bookingUse="groupedData"
      ...
      <div :class="`d-flex flex-row justify-content-between footer-color`">
          <div class="col-6 p-0 d-flex align-items-center ma-2">
            <v-icon class="ml-6" large>
             ...
             <span class="text-footer ml-3" style="opacity: 0.9;">POWER BY BETASK CONSULTING</span>
             <!-- <v-btn icon
                      large
                      color="#695988"
...
export default {
  ...
  computed: {
    ...
    sortedDataQueueList () {
      ...
      const queueData = this.itemBookingUse.filter(item => item.queue !== '' && item.masBranchName !== '')
      const emptyData = this.itemBookingUse.filter(item => item.queue === '' || item.masBranchName === '')
