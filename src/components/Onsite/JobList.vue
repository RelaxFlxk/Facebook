            ...
            <v-col cols="12">
                ...
                <v-tabs-items v-model="tab" v-if="itemJob.length > 0">
                    <v-tab-item
                        ...
                        :key="indexitem"
                    >
                    <div v-for="(items, indexitems) in itemJob.filter(el => { return el.stepId === item.value })" :key="indexitems">
                        <v-card
                        class="ma-2 p-1"
                        width="340"
                        height="160"
                        ...
                        >
                            <v-row>
                                ...
                                <v-row class="font12 headline1" v-for="(itemJobs, indexitemJob) in dataJob.filter(el => { return el.jobNo === items.jobNo })" :key="indexitemJob">
                                    <v-col class="pl-0 pt-0 pb-0">{{itemJobs.fieldName + ' : ' + itemJobs.fieldValue}}</v-col>
                                ...
                                <v-row class="font12 headline1">
                                    <v-col class="pl-0 pt-0 pb-0">วันที่นัดหมาย : {{items.dueDateFomatTH}}</v-col>
                                </v-row>
                                <v-row class="font12 headline1">
                                    <v-col class="pl-0 pt-0 pb-0">วันเวลาที่เริ่ม : {{items.onsiteStartDateFomatTH}}</v-col>
                                </v-row>
...
export default {
  ...
  methods: {
    async getDataFlow () {
      this.itemFlow = await this.getDataFromAPI('/flow/get', 'flowId', 'flowName', '&checkOnsite=True')
    },
    async getDataFlowStep () {
      this.itemFlowStep = await this.getDataFromAPI('/flowStep/get', 'stepId', 'stepTitle', '&flowId=' + this.selectFlow)
    },
    ...
    async checkEmpJob () {
      ...
      await axios
        .get(
          this.DNS_IP + '/job/getList?shopId=' + this.session.data.shopId + '&empStep=' + this.session.data.empId + '&flowId=' + this.selectFlow
        )
        .then(async response => {
          let rs = response.data
          if (rs.status === false) {
    ...
    async getJobData () {
      ...
      await axios
        .get(
          this.DNS_IP + '/job/get?shopId=' + this.session.data.shopId + '&empStepId=' + this.session.data.empId + '&flowId=' + this.selectFlow
        )
        .then(async response => {
          let rs = response.data
          if (rs.status === false) {
      ...
      this.$swal(this.swalConfig)
        .then(async () => {
          if (this.$session.id() !== undefined) {
            var dt = {}
            var stepIdSelect = this.itemFlowStep.filter(el => { return el.allData.sortNo === sortNo + 1 })[0].value
            if (sortNo === 1) {
              dt = {
                stepId: stepIdSelect,
                onsiteStartDate: moment(moment(new Date()), 'YYYY-MM-DD HH:mm:ss').format('YYYY-MM-DD HH:mm:ss'),
            ...
            await axios
              .post(
                this.DNS_IP + '/job/editAll/' + jobId, dt
              ).then(async (response) => {
